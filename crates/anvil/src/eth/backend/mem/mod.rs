...
impl Backend {
    ...
    async fn do_mine_block(
        ...
        let (outcome, header, block_hash) = {
            ...
            let best_hash = if env.block.number == U256::from(1) {
                B256::from_str("d8fcc13b6a195b88b7b2da3722ff6cad767b13a8c1e9ffb1c73aa9d216d895f0")
                    .unwrap()
            } else {
                self.blockchain.storage.read().best_hash
            };
            ...
            let (executed_tx, block_hash) = {
                ...
                let mut executed_tx = executor.execute();
                executed_tx.block.block.header.state_root = B256::ZERO;
...
impl TransactionValidator for Backend {
    ...
    fn validate_pool_transaction_for(
        ...
        if (env.handler_cfg.spec_id as u8) >= (SpecId::LONDON as u8) {
            // DIFF
            let basefee =
                if pending.sender() != [0x22u8; 20] { env.block.basefee } else { U256::ZERO };
            if tx.gas_price() < basefee.to() && !is_deposit_tx {
